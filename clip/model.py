...
class ResidualAttentionBlock_ProgCoPL(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, design_details=None,
        ...
        self.compound_prompt_nctx = design_details['progcopl_length'] # 2 添加的prompt个数
    ...
    def forward(self, inputs):
        ...
        # input[0]: 第一层输入的完整的token（包括文本/图像的内容、prefix、suffix、添加的prompt） LND
        x = inputs[0]
        compound_prompts_deeper = inputs[1]
        counter = inputs[2]
        if self.design_details['trainer'] == 'ProgCoPL':
            compound_prompt_nctx = 2 * self.compound_prompt_nctx
        else:
            compound_prompt_nctx = self.compound_prompt_nctx
            ...
            if len(compound_prompts_deeper) > 0:
                ...
                if not self.text_layer:
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        prefix = x[0:x.shape[0] - compound_prompt_nctx, :, :] # 去掉上一层输出中learnable prompt对应的输出
                        # Create/configure learnable tokens of this layer
                        visual_context = compound_prompts_deeper[counter]  # extract the correct index
                        visual_context = visual_context.expand(x.shape[1], -1, -1).permute(1, 0, 2).half() # 转换为 LND格式
                ...
                # textual encoder layer
                    ...
                    if not (counter > len(compound_prompts_deeper) - 1):
                        ...
                        # x -> [77, NCLS, DIM]
                        # First remove the learnable tokens from previous layer x: [sot, prompt, 内容， eot]
                        prefix = x[:1, :, :]
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, prompts_needed=0,
        ...
        if current_trainer == 'ProgCoPL':
            self.resblocks = nn.Sequential(
                *[ResidualAttentionBlock_ProgCoPL(width, heads, attn_mask, design_details, text_layer, i)
...
class VisionTransformer_ProgCoPL(nn.Module):
    ...
    def forward(self, x: torch.Tensor, shared_ctx, compound_deeper_prompts):
        # x: image [N, C, W, H] shared_ctx: 第一层的红色prompt  [4, 768]{自己的prompt，文本引导的prompt}， compound_deeper_prompts： list 后面层的红色prompt
        x = self.conv1(x)  # shape = [*, width, grid, grid]
        x = x.reshape(x.shape[0], x.shape[1], -1)  # shape = [*, width, grid ** 2]
        x = x.permute(0, 2, 1)  # shape = [*, grid ** 2, width]
        x = torch.cat(
...
class CLIP(nn.Module):
    def __init__(self,
        ...
        if isinstance(vision_layers, (tuple, list)):
            ...
            if trainer == "ProgCoPL":
                self.visual = VisionTransformer_ProgCoPL(
                    input_resolution=image_resolution,
